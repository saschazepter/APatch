...
use crate::utils::{switch_cgroups,ensure_dir_exists,get_work_dir};
use crate::{
...
pub fn on_post_data_fs(superkey: Option<String>) -> Result<()> {
    ...
    if utils::should_enable_overlay()? {
        ...
        copy_dir_with_xattr(Path::new(module_dir), Path::new(get_work_dir().as_str()))?;
        if let Err(e) = mount_systemlessly(&get_work_dir()) {
